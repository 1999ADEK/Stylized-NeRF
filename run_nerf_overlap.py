...
def config_parser():
    ...
    parser.add_argument("--i_testset", type=int, default=50000000, 
                        help='frequency of testset saving')
    parser.add_argument("--i_video",   type=int, default=50000000, 
    ...
    parser.add_argument("--patch_size",   type=int, default=0, 
                        help='patch size, 0 to use original size')
...
def train():
    ...
    # Prepare for stylization
    (h, w) = tuple([int((float(args.patch_size) / max([H,W]))*x) for x in [H, W]]) \
                             if args.patch_size else (H, W)
    style_tensor = get_style_tensor(args.style_path, size=min(h, w))
    ...
    for i in trange(start, args.N_iters):
        ...
        if use_batching:
            ...
            ih = np.random.randint(h*args.patch_num-h+1)
            iw = np.random.randint(w*args.patch_num-w+1)
            ...
            if N_rand is not None:
                ...
                if i < args.precrop_iters:
                    ...
                    coords = torch.stack(
                        torch.meshgrid(torch.linspace(ih, ih+h-1, h)*H/(args.patch_num*h), torch.linspace(iw, iw+w-1, w)*W/(args.patch_num*w)), 
                        -1
                        )  # (h, w, 2)
                ...
                target_s = target_s[:, :, ih:ih+h, iw:iw+w]  # (1, 3, h, w)
        ...
        if i%args.i_print==0:
            tqdm.write(f"[TRAIN] Iter: {i} Loss: {loss['total'].item()}  PSNR: {psnr.item()}")
        
            #print(expname, i, psnr.numpy(), loss.numpy(), global_step.numpy())
            #print('iter time {:.05f}'.format(dt))

            writer.add_scalar('loss/content', loss['content'], i)
            writer.add_scalar('loss/style', loss['style'], i)
            writer.add_scalar('loss/tv', loss['tv'], i)
            ...
            if i%args.i_img==0:
                ...
                writer.add_image('rgb', to8b(rgb.cpu().numpy()).transpose(2,0,1), i)
                writer.add_image('disp', disp.unsqueeze(0), i)
                writer.add_image('acc', acc.unsqueeze(0), i)

                #writer.add_scalar('psnr_holdout', psnr, i)
                writer.add_image('rgb_holdout', target.cpu().numpy().transpose(2,0,1), i)


                if args.N_importance > 0:

                    writer.add_image('rgb0', to8b(extras['rgb0'].cpu().numpy()).transpose(2,0,1), i)
                    writer.add_image('disp0', extras['disp0'].unsqueeze(0), i)
                    writer.add_image('z_std', extras['z_std'].unsqueeze(0), i)
